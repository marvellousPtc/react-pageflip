...
export abstract class Render {
    ...
    private calculateBoundsRect(): Orientation {
        ...
        const singlePage = this.app.getSettings().singlePage;
        ...
        if (this.setting.size === SizeType.STRETCH) {
            if (singlePage || (blockWidth < this.setting.minWidth * 2 && this.app.getSettings().usePortrait))
                orientation = Orientation.PORTRAIT;

            pageWidth =
                orientation === Orientation.PORTRAIT
                    ? (singlePage ? this.getBlockWidth() / 2 : this.getBlockWidth())
            ...
            } else if (blockWidth < pageWidth * 2) {
